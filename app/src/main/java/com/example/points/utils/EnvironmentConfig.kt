...
object EnvironmentConfig {
    ...
    fun initialize(context: Context) {
        try {
            ...
            // Intentar copiar .env desde assets a filesDir
            try {
                ...
                // Si no existe en assets, usar el existente en filesDir (si existe)
            }
            
            // Cargar variables de entorno desde filesDir
            dotenv = dotenv {
            ...
            android.util.Log.e("EnvironmentConfig", "Error al inicializar variables de entorno: ${e.message}")
    ...
    // Detection API (Servidor de IA para detección de amenazas)
    val DETECTION_API_URL: String
        get() = getEnvValue("DETECTION_API_URL")
    
    val DETECTION_API_KEY: String
        get() = getEnvValue("DETECTION_API_KEY")
